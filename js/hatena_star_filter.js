...
class HatenaStarFilter {
    ...
    request_star_json(username, anchor, elem_stars, callback) {
        ...
        MessageUtil.send_message(
            {command:MessageUtil.command_get_hatena_json(),
             anchor: anchor,
             username: username});
    }
    ...
    reply_hatenaAPI_entry(message) {
        ...
        var filtering_stars = (stars, color)=> {
            var stars_buff = [];
            for (const star of stars) {
                if (star.count != null) {
        ...
        // normal star
        filtering_stars(entry.stars, 'yellow');
        // color star
        if (entry.colored_stars != null) {
            for (const c_stars of entry.colored_stars) {
                filtering_stars(c_stars.stars, c_stars.color);
            }
        }
